...
// EnvelopeEncryptAes performs envelope encryption with one-off symmetric key
// using AES-GCM as symmetric encryption algorithm and RSA-OAEP as asymmetric one
//
// Note: Asymmetric key is in PEM format
func EnvelopeEncryptAes(message string, publicKeyPem string) (string, string, error) {
	ephemeralKey, err := Key256b()
	if err != nil {
		return "", "", fmt.Errorf("EnvelopeEncrypt failed to generate symmetric key with error: [%w]", err)
	}
	ciphertext, err := EncryptAesGcm(message, ephemeralKey)
	if err != nil {
		return "", "", fmt.Errorf("EnvelopeEncrypt failed to encrypt message with error: [%w]", err)
	}
	encryptedSymmetricKey, err := EncryptRsa(ephemeralKey, publicKeyPem)
	if err != nil {
		return "", "", fmt.Errorf("EnvelopeEncrypt failed to encrypt ephemeral key with error: [%w]", err)
	}
	return ciphertext, encryptedSymmetricKey, nil
}

// EnvelopeDecryptAes performs decryption of ciphertext generated by EnvelopeEncryptAes
// Note: encryptedSymmetricKey has to be decryptable using privateKeyPem
//
// Note: Asymmetric key is in PEM format
func EnvelopeDecryptAes(ciphertext string, encryptedSymmetricKey string, privateKeyPem string) (string, error) {
	ephemeralSymmetricKey, err := DecryptRsa(encryptedSymmetricKey, privateKeyPem)
	if err != nil {
		return "", fmt.Errorf("EnvelopeDecryptAes failed to decrypt symmetric key with error: [%w]", err)
	}
	plaintext, err := DecryptAesGcm(ciphertext, ephemeralSymmetricKey)
	if err != nil {
		return "", fmt.Errorf("EnvelopeDecryptAes failed to decrypt ciphertext with error: [%w]", err)
	}
