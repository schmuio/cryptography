...
<br>
<br>
The problem is that SHA1 has been long proven to be fundamentally insecure and is no longer recommended by NIST [2], and evidence has been growing it is even more flawed with respect to collision resistance than previously thought [3,4]. However, admittedly it has been "relatively safe" in other contexts. [4]. For example, in TOTP generation collision resitance is not a required property as well as only a small 6-digit part of the whole hash is used so that the generic collision attacks do not seem to be particularlly applicable.
<br>
<br>
