...
<br>
<br>
The problem is that for long time SHA1 has been proven to be fundamentally insecure and is no longer recommended by NIST [2], and evidence has been growing it is even more flawed with respect to collision resistance than previously thought [ [3](https://duo.com/decipher/sha-1-fully-and-practically-broken-by-new-collision) ], [ [4](https://eprint.iacr.org/2020/014.pdf) ]. However, reportedly it has been "relatively safe" in other contexts [ [4](https://eprint.iacr.org/2020/014.pdf) ]. For example, in TOTP generation collision resitance is not a required property as well as only a small 6-digit part of the whole hash is used so that the generic collision attacks do not seem to be particularlly applicable.
<br>
<br>
